...
typedef struct 
{
    cl_command_queue    CmdQue[4];
    cl_kernel           imgKernel[4];
...
typedef struct 
{
    ...
    cl_kernel           imgKernel[Cmd_Que_Num];
    cl_program          program[Cmd_Que_Num]; 
    cl_mem              cl_outputImg[Cmd_Que_Num];
    void*               Ptr_out[Cmd_Que_Num];
}CLoutOBJ;
...
int init_cl_mem_obj(CLpltOBJ &pltobj, CLmemOBJ &memobj, Mat map_x, Mat map_y, void *GC50UT_ptr[])
{
    ...
    CHECK_ERROR(status, "clCreateMapTable failed.");
    memobj.outputGray = clCreateBuffer(pltobj.context,CL_MEM_WRITE_ONLY || CL_MEM_ALLOC_HOST_PTR, data_size * sizeof(float),NULL,&status);
    CHECK_ERROR(status, "clCreateoutGray failed.");
    ...
    clmemobj.mapGrayptr = clEnqueueMapBuffer( clmemobj.CmdQue[3], clmemobj.outputGray, CL_TRUE, CL_MAP_READ,
        0, 180 * 260 * sizeof(float), 0, NULL, NULL, NULL);
...
int gpu7k_get_viraddr(void* addrarray_f[], void* addrarray_r[], int size, int addr_cnt)
{
    ...
    ifstream input2( "Map_Fx.txt", ios::in | ios::binary );
    if( ! input2 )
    {
        cerr << "Open input file error!" << endl;
        exit( -1 );
    }
    input2.read( ( char * )Map_Fx.data , sizeof( float ) * Map_tab_wid * Map_tab_hei);
    
    
    ifstream input3( "Map_Fy.txt", ios::in | ios::binary );
    if( ! input3 )
    {
        cerr << "Open input file error!" << endl;
        exit( -1 );
    }
    input3.read( ( char * )Map_Fy.data , sizeof( float ) * Map_tab_wid * Map_tab_hei);
    ...
    status |= init_cl_mem_obj(clpltobj, clmemobj, Map_Rx, Map_Ry, addrarray_f);
    status |= init_cl_mem_obj(clpltobj, clmemobj_r, Map_Fx, Map_Fy, addrarray_r);
...
int init_cl_affine_obj(CLpltOBJ &pltobj, CLaffineOBJ &memobj, void * ptr[])
{
    ...
    memobj.CmdQue[2] = clCreateCommandQueue(pltobj.context, pltobj.devices[0], 0, &status);
    CHECK_ERROR(status, "clCreateCommamdQueue failed.");
    memobj.program[2] = clCreateProgramWithSource(pltobj.context, 1, &source, sourceSize, &status);
    status = clBuildProgram(memobj.program[2], 1, pltobj.devices, NULL, NULL, NULL);
    CHECK_ERROR(status, "clCreateProgarm failed.");
    memobj.imgKernel[2] = clCreateKernel(memobj.program[2], "imageMergef", &status);
    CHECK_ERROR(status, "clCreateKernel failed.");

    memobj.CmdQue[3] = clCreateCommandQueue(pltobj.context, pltobj.devices[0], 0, &status);
    CHECK_ERROR(status, "clCreateCommamdQueue failed.");
    memobj.program[3] = clCreateProgramWithSource(pltobj.context, 1, &source, sourceSize, &status);
    status = clBuildProgram(memobj.program[3], 1, pltobj.devices, NULL, NULL, NULL);
    CHECK_ERROR(status, "clCreateProgarm failed.");
    memobj.imgKernel[3] = clCreateKernel(memobj.program[3], "imageMerger", &status);
    CHECK_ERROR(status, "clCreateKernel failed.");
    ...
    argIdx = 0;
    status |= clSetKernelArg(memobj.imgKernel[2], argIdx++, sizeof(cl_mem), &clmemobj.outputImgMap);
    status |= clSetKernelArg(memobj.imgKernel[2], argIdx++, sizeof(cl_mem), &memobj.inputImgMap);
    argIdx = 0;
    status |= clSetKernelArg(memobj.imgKernel[3], argIdx++, sizeof(cl_mem), &clmemobj.outputImgMap);
    status |= clSetKernelArg(memobj.imgKernel[3], argIdx++, sizeof(cl_mem), &memobj.inputImgMap);

    
    return status;
}

int init_cl_out_obj(CLpltOBJ &pltobj, CLoutOBJ &memobj);

int init_cl_Affine(void * affinePtr[])
{
    init_cl_affine_obj(clpltobj, affineobj, affinePtr);
    init_cl_out_obj(clpltobj, outbufobj);
}
...
int init_cl_out_obj(CLpltOBJ &pltobj, CLoutOBJ &memobj)
{
    ...
    for(int i = 0; i < Cmd_Que_Num; i++)
    {
        ...
        status = clBuildProgram(memobj.program[i], 1, pltobj.devices, NULL, NULL, NULL);
        CHECK_ERROR(status, "clCreateProgarm failed.");
        memobj.imgKernel[i] = clCreateKernel(memobj.program[i], "outCopy", &status);
        CHECK_ERROR(status, "clCreateKernel failed.");
    }
    ...
    for(int i = 0; i < Cmd_Que_Num; i++)
    {
        memobj.cl_outputImg[i] = clCreateImage(pltobj.context, CL_MEM_READ_WRITE || CL_MEM_ALLOC_HOST_PTR, &img_format, &pixelDesc, NULL, &status);
        CHECK_ERROR(status, "clCreateImage2D failed. (inputImgMap)");

        memobj.Ptr_out[i] = clEnqueueMapImage( memobj.CmdQue[i], memobj.cl_outputImg[i], CL_TRUE, CL_MAP_READ, 
        imageOrigin, imageRegion, &imageRowPitch_in, NULL, 0, NULL, NULL, &status);
        CHECK_ERROR(status, "clEnqueueMapBuffer failed. (resultBuf)");
    }
    ...
    {
        int argIdx = 0;
        status |= clSetKernelArg(memobj.imgKernel[i], argIdx++, sizeof(cl_mem), &affineobj.inputImgMap);
        status |= clSetKernelArg(memobj.imgKernel[i], argIdx++, sizeof(cl_mem), &memobj.cl_outputImg[i]);
    }
...
//    clFinish(affineobj.CmdQue[3]);
    ...
    static size_t   globaworksize_2[2];
    globaworksize_2[0] = Affine_W;
    globaworksize_2[1] = Affine_H;
